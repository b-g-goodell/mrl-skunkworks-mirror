...
class TestSimulatorDeeply(unittest.TestCase):
    ...
    def test_spend_from_buffer(self):
        ...
        sally.buffer[sally.t] = [(owner, recip, node_to_spend)] # Ignore anything else placed in the buffer here so far.

        num_left_nodes = len(sally.g.left_nodes)
        num_right_nodes = len(sally.g.right_nodes)

        sally.spend_from_buffer() # Should add exactly one new right_node, two new left nodes
    ...
    def test_get_ring_correctness(self):
        ...
        self.assertEqual(len(ring), sally.ringsize)
...
class TestSimulatorLite(unittest.TestCase):
    """ TestSimulator tests our simulator """
    def test_init(self):
        sally = make_sally()
        self.assertEqual(sally.runtime, RUNTIME)
    ...
    def test_spend_from_buffer(self):
        ...
        sally.spend_from_buffer() # Should add exactly one new right_node, two new left nodes, sally.ringsize red edges, and 2 blue edges.

        self.assertEqual(len(sally.g.left_nodes), num_left_nodes+2)
        self.assertEqual(len(sally.g.right_nodes), num_right_nodes+1)

tests = [TestSimulatorDeeply, TestSimulatorLite]
# tests = [TestSimulatorLite]
for test in tests:
